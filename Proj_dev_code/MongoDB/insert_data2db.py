    ...
    if folders == "cash_flow" :
        ...
        for files in os.listdir(lasted_data+"/%s"%folders):
            ...
            if company_exist !=0 : #updata
                Cash_Flow_Fn.updata_company_db(company_number,company_name,file_data.read().split("\n"))
            else : #creat 
                Cash_Flow_Fn.creat_company_db(company_number,company_name,file_data.read().split("\n"))
            file_data.close()
    if folder == "" :
